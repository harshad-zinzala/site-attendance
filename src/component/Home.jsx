...
export default function Home({ 
  ...
  const [step1Count, setStep1Count] = useState(7);
  ...
  const handleToggleAccordion = (deptId) => {
    if (expandedDeptId === deptId) {
      ...
      if (dept) {
        // Default count to the remaining unassigned workers (min 1, max unassigned)
        setStep1Count(dept.unassigned > 0 ? dept.unassigned : dept.totalWorkers);
      }
    }
  };
  ...
  const handleMarkAttendance = (deptId) => {
    ...
    // Reset step 1 count to remaining unassigned
    const remainingUnassigned = Math.max(0, dept.unassigned - step1Count);
    setStep1Count(remainingUnassigned > 0 ? remainingUnassigned : dept.totalWorkers);
  };
