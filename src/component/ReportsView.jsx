            ...
            <div className="p-5 overflow-y-auto space-y-5">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {[
                    { name: "Mason", iconColor: "bg-orange-100 text-orange-700", head: "Ramesh Kumar" },
                    { name: "Electrical", iconColor: "bg-amber-100 text-amber-700", head: "Suresh Patel" },
                    { name: "Plumbing", iconColor: "bg-blue-100 text-blue-700", head: "Amit Singh" },
                    { name: "Fabrication", iconColor: "bg-teal-100 text-teal-700", head: "Vikram Rao" },
                    { name: "Waterproofing", iconColor: "bg-emerald-100 text-emerald-700", head: "Karan Johar" }
                  ].map((dept, index) => {
                    // Distribute stats proportionally for a realistic display
                    const p = Math.ceil(selectedReport.present * (0.3 - index * 0.05));
                    const h = Math.ceil(selectedReport.halfDay * (0.25 - index * 0.05));
                    const l = Math.ceil(selectedReport.leave * (0.2 - index * 0.05));
                    
                    const actualP = Math.max(0, p);
                    const actualH = Math.max(0, h);
                    const actualL = Math.max(0, l);
