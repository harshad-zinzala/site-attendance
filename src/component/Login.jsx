...
import { Mail, Lock, Sparkles, AlertCircle, Shield, User } from 'lucide-react';
...
export default function Login({ onLoginSuccess, showToast }) {
  ...
  const handleLogin = async (e) => {
    ...
    if (isSupabaseConfigured) {
      ...
      setIsLoading(false);
      if (email === 'admin@atpl.com' && password === 'admin123') {
        onLoginSuccess({ id: 'demo-admin-id', email }, 'master_admin');
        showToast('🎉 Logged in as Master Admin (Demo)!');
      } else if (email === 'sub@atpl.com' && password === 'sub123') {
        onLoginSuccess({ id: 'demo-sub-id', email }, 'sub_admin');
        showToast('🎉 Logged in as Sub Admin (Demo)!');
      } else {
        // Allow dynamic mock user creation in Demo mode if SignUp is toggled
        setErrorMsg('Invalid demo credentials. Use the quick buttons below or Admin/Sub Admin defaults.');
      }
    }
  };
  ...
  const handleSignUp = async (e) => {
    ...
    if (isSupabaseConfigured) {
      try {
        ...
        // Note: handle_new_user database trigger will automatically create the profile row.
      ...
      showToast('🎉 Demo account registered! You can now log in.');
      setIsSignUp(false);
    }
  };
  ...
  const handleQuickLogin = (role) => {
    if (role === 'master_admin') {
      setEmail('admin@atpl.com');
      setPassword('admin123');
      onLoginSuccess({ id: 'demo-admin-id', email: 'admin@atpl.com' }, 'master_admin');
      showToast('🎉 Logged in as Master Admin (Demo)!');
    } else {
      setEmail('sub@atpl.com');
      setPassword('sub123');
      onLoginSuccess({ id: 'demo-sub-id', email: 'sub@atpl.com' }, 'sub_admin');
      showToast('🎉 Logged in as Sub Admin (Demo)!');
    }
  };
        ...
        <div className="bg-white rounded-3xl p-6 border border-slate-200 shadow-xl space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-3">
            ...
            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${
              isSupabaseConfigured ? 'bg-emerald-50 text-emerald-700' : 'bg-orange-50 text-orange-700'
            }`}>
