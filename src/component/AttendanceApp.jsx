...
export default function AttendanceApp() {
  ...
  const fetchSubAdmins = async () => {
    if (isSupabaseConfigured) {
      ...
      // Seed initial mock sub admins if empty
      setSubAdmins(prev => prev.length === 0 ? [{ id: 'demo-sub-id', email: 'sub@atpl.com' }] : prev);
    }
  };
  ...
  const handleAddSubAdmin = async (e) => {
    ...
    if (isSupabaseConfigured) {
      ...
      const newSub = { id: `demo-sub-${Date.now()}`, email: newSubEmail };
      setSubAdmins(prev => [...prev, newSub]);
      showToast(`👤 Sub Admin ${newSubEmail} added (Demo)!`);
      setNewSubEmail('');
      setNewSubPassword('');
    }
  };
  ...
  const handleDeleteSubAdmin = async (subId, subEmail) => {
    if (isSupabaseConfigured) {
      ...
      setSubAdmins(prev => prev.filter(a => a.id !== subId));
      showToast(`Deleted Sub Admin: ${subEmail} (Demo)`);
    }
  };
  ...
  const bulkUpdateWorkerStatus = async (deptId, count, status) => {
    ...
    // 2. Sync to DB if enabled AND today is selected
    if (isDbMode && isSelectedDateToday()) {
      try {
                ...
                <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                  ...
                  {userRole === 'master_admin' && (
                    <button 
                      ...
                      className="w-full px-4 py-3 flex items-center justify-between text-xs text-rose-600 hover:bg-rose-50/50 font-bold border-b border-slate-100"
                    >
