...
export default function AttendanceApp() {
  ...
  // Prefill profile email when user session changes
  useEffect(() => {
    if (user) {
      setNewProfileEmail(user.email || '');
    }
  }, [user]);
  ...
  const handleUpdateProfile = async (e) => {
    ...
    try {
      const updates = {};
      if (newProfileEmail && newProfileEmail !== user.email) {
        updates.email = newProfileEmail;
      }
      if (newPassword) {
      ...
      // Sync new email to user_roles table if changed
      if (updates.email) {
        const { error: roleError } = await supabase
          ...
          .eq('id', user.id);
        if (roleError) {
          console.error("Failed to sync email to user_roles:", roleError.message);
        }
      }

      if (updates.email && updates.password) {
        showToast("🎉 Profile updated! Check new email for confirmation.");
      } else if (updates.email) {
        showToast("🎉 Email update requested! Check email for confirmation.");
      } else {
              ...
              <div className="p-5 space-y-4 h-full bg-slate-50/20 pb-[150px]">
                ...
                <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm">
                  <button 
                    ...
                    className="w-full px-4 py-3 flex items-center justify-between text-xs text-slate-700 hover:bg-slate-50 border-b border-slate-100 transition-colors"
                  >
                    <span className="font-semibold">User Profile Settings</span>
                    <ChevronDown size={14} className={`text-slate-400 transition-transform duration-200 ${isProfileExpanded ? '' : '-rotate-90'}`} />
                  </button>
                      ...
                      <form onSubmit={handleUpdateProfile} className="space-y-3">
                        <div>
                          ...
                          <input 
                            ...
                            required
                            className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-xs focus:outline-none focus:border-orange-500 text-slate-800"
                          />
                        </div>
                        <div>
                          <label className="text-[10px] text-slate-400 font-bold uppercase block mb-1">New Password (Optional)</label>
                          <input 
                            type="password" 
                        ...
                        <button 
                          type="submit"
                          className="w-full bg-orange-600 hover:bg-orange-700 text-white font-bold text-xs py-2.5 rounded-xl cursor-pointer transition active:scale-98"
                        >
                          Update Profile
                        </button>
                      </form>
